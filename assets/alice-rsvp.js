(function () {
  ...
  function renderLocalTable() {
    ...
    if (!rows.length) {
      ...
      tr.innerHTML = "<td colspan='9'>Nenhuma resposta salva neste navegador.</td>";
      body.appendChild(tr);
      return;
    }
    ...
    rows.forEach((row) => {
      ...
      tr.innerHTML = `
        ...
        <td>${row.total || 0}</td>
        <td>${row.presente || "-"}</td>
        <td>${row.tamanhoFralda || "-"}</td>
        <td>${Number.isNaN(date.getTime()) ? "-" : date.toLocaleString("pt-BR")}</td>
      `;
      body.appendChild(tr);
    });
  }
  ...
  function downloadCsv() {
    ...
    link.download = "respostas-cha-alice.csv";
    link.click();
